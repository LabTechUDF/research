...
KPIs (Key Performance Indicators) são medidas quantitativas utilizadas para avaliar a eficiência e eficácia de um processo ou projeto. Eles fornecem uma visão clara do desempenho e permitem identificar áreas que necessitam de melhorias, facilitando a tomada de decisões baseadas em dados.

\subsubsection{Interface de Programação de Aplicações (API)}
Uma API (Application Programming Interface) é um conjunto de protocolos e rotinas que possibilita a comunicação e integração entre diferentes sistemas. No contexto deste projeto, a API do GitHub será utilizada para coletar automaticamente dados relevantes dos repositórios, como \emph{commits}, \emph{issues} e \emph{pull requests}.

\subsubsection{Versionador e Repositório Remoto}
Um versionador, como o Git, é um sistema de controle de versões que registra todas as alterações realizadas no código-fonte ao longo do tempo, possibilitando rastreamento e reversão de mudanças. Por sua vez, um repositório remoto, como o GitHub, é um serviço online que hospeda esses arquivos versionados, permitindo a colaboração entre desenvolvedores e o acesso remoto ao código.
