...
% Controle para omitir as últimas partes (apêndices / anexos).
% Defina \omitlastpages=1 para OMITIR apêndices e anexos (gera PDF sem as últimas ~20 páginas).
% Defina \omitlastpages=0 para INCLUIR novamente.
\newcount\omitlastpages
\omitlastpages=1 % 1 = omitir; 0 = incluir
...
\usepackage{lmodern}                                % Latin Modern fonts (reduces font-shape warnings)
\usepackage{inconsolata}                            % better monospaced font (reduces lmtt warnings)
...
\begin{OnehalfSpacing}
    ...
    % Elementos textuais
    \textual
    \include{./02-elementos-textuais/introducao}            % Introdução (Cap 1, 2, 3-Tecnologias)
    \include{./02-elementos-textuais/metodologia}           % Metodologia (Cap 3)
    \include{./02-elementos-textuais/desenvolvimento}       % Desenvolvimento (Cap 4)
...
\end{OnehalfSpacing}
        ...
        \begin{OnehalfSpacing}
        % As duas inclusões abaixo (apêndices e anexos) podem ser omitidas
        % definindo \omitlastpages=1 no preâmbulo (padrão aqui). Para incluir,
        % defina \omitlastpages=0.
        \ifnum\omitlastpages=0
